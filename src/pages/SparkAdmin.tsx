...
import { Plus, Pencil, Trash2, Eye, EyeOff, LayoutDashboard, Calendar, FileText, Image, BarChart3, Users, BookOpen, X } from 'lucide-react';
  ...
  if (!user) return (
    <div className="min-h-screen flex items-center justify-center font-[Nunito] bg-gray-50">
      <div className="text-center">
        <img src={sparkLogo} alt="Spark" className="h-16 w-16 mx-auto mb-6" />
        <h1 className="text-2xl font-extrabold text-gray-900 mb-2">Spark Admin</h1>
        <p className="text-gray-400 mb-6">Please sign in to access the admin panel.</p>
        <Link to="/auth" className="bg-[#CCFF00] text-gray-900 font-extrabold text-sm tracking-[0.08em] uppercase px-8 py-3 rounded-full hover:bg-[#B8E600] transition-all">Sign In</Link>
      </div>
    </div>
  );
  ...
  if (!isAdmin) return (
    <div className="min-h-screen flex items-center justify-center font-[Nunito] bg-gray-50">
      <div className="text-center">
        ...
        <p className="text-gray-400 mb-6">You don't have admin privileges for Spark.</p>
  ...
  return (
    <>
      ...
      <div className="bg-gray-50 text-gray-900 min-h-screen font-[Nunito]">
        <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-3 bg-gray-900 text-white">
          <div className="flex items-center gap-3">
            <img src={sparkLogo} alt="Spark" className="h-8 w-8" />
            <span className="text-sm font-black tracking-[0.15em] uppercase">Spark Admin</span>
          </div>
          <div className="flex items-center gap-4">
            <span className="text-xs text-gray-400">{user.email}</span>
            <Link to="/spark" className="text-[11px] tracking-[0.1em] uppercase font-bold text-[#CCFF00] hover:text-[#B8E600] transition-colors">View Site →</Link>
          </div>
        </nav>

        <div className="pt-16 flex">
          {/* Sidebar */}
          <aside className="w-56 fixed left-0 top-16 bottom-0 bg-white border-r border-gray-100 p-4 space-y-1">
            {tabs.map(tab => (
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {stats.map(s => (
          <div key={s.label} className="bg-white rounded-xl p-6 border border-gray-100">
            ...
            <p className="text-[11px] uppercase tracking-[0.1em] text-gray-400 font-bold">{s.label}</p>
          </div>
        ))}
      </div>
      <div className="bg-white rounded-xl p-6 border border-gray-100">
        ...
        <div className="flex flex-wrap gap-3">
          <button className="text-[11px] tracking-[0.1em] uppercase font-bold px-4 py-2 rounded-lg bg-[#CCFF00]/10 text-[#65A300] hover:bg-[#CCFF00]/20 transition-colors">+ Create Event</button>
          <button className="text-[11px] tracking-[0.1em] uppercase font-bold px-4 py-2 rounded-lg bg-[#7B61FF]/10 text-[#7B61FF] hover:bg-[#7B61FF]/20 transition-colors">+ Write Post</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-extrabold">Spark Events</h2>
        <button onClick={() => { resetForm(); setEditingEvent(null); setShowForm(true); }} className="flex items-center gap-2 bg-[#CCFF00] text-gray-900 text-[11px] font-extrabold tracking-[0.1em] uppercase px-4 py-2.5 rounded-lg hover:bg-[#B8E600] transition-colors">
          <Plus className="w-4 h-4" /> New Event
        </button>
      </div>

      {showForm && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl p-6 border border-gray-100 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold">{editingEvent ? 'Edit Event' : 'Create Event'}</h3>
          ...
          <div className="flex gap-3 mt-4">
            <button onClick={() => saveMutation.mutate()} disabled={!form.title || !form.date} className="bg-[#CCFF00] text-gray-900 text-[11px] font-extrabold tracking-[0.1em] uppercase px-6 py-2.5 rounded-lg hover:bg-[#B8E600] transition-colors disabled:opacity-50">
      ...
      <div className="space-y-2">
        {events.map((e: any) => (
          <div key={e.id} className="bg-white rounded-xl px-6 py-4 border border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-4">
              {e.published ? <Eye className="w-4 h-4 text-[#65A300]" /> : <EyeOff className="w-4 h-4 text-gray-300" />}
              <div>
                <p className="font-bold text-sm">{e.title}</p>
                <p className="text-[11px] text-gray-400">{e.event_type} · {e.date ? format(new Date(e.date), 'MMM dd, yyyy') : 'No date'}</p>
              </div>
            </div>
        ...
        {events.length === 0 && <p className="text-center text-gray-400 py-8">No events yet. Create your first one!</p>}
      </div>
    </div>
  );
};
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { resetForm(); setEditingPost(null); setShowForm(true); }} className="flex items-center gap-2 bg-[#CCFF00] text-gray-900 text-[11px] font-extrabold tracking-[0.1em] uppercase px-4 py-2.5 rounded-lg hover:bg-[#B8E600] transition-colors">
      ...
      {showForm && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl p-6 border border-gray-100 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold">{editingPost ? 'Edit Post' : 'Create Post'}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <textarea placeholder="Content (full article)" value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} className="px-3 py-2 border border-gray-200 rounded-lg text-sm md:col-span-2" rows={8} />
          ...
          <div className="flex gap-3 mt-4">
            <button onClick={() => saveMutation.mutate()} disabled={!form.title || !form.content} className="bg-[#CCFF00] text-gray-900 text-[11px] font-extrabold tracking-[0.1em] uppercase px-6 py-2.5 rounded-lg hover:bg-[#B8E600] transition-colors disabled:opacity-50">
      ...
      <div className="space-y-2">
        {posts.map((p: any) => (
          <div key={p.id} className="bg-white rounded-xl px-6 py-4 border border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-4">
              {p.published ? <Eye className="w-4 h-4 text-[#65A300]" /> : <EyeOff className="w-4 h-4 text-gray-300" />}
              <div>
                <p className="font-bold text-sm">{p.title}</p>
                <p className="text-[11px] text-gray-400">{p.category} · {p.author_name}{p.published_at ? ` · ${format(new Date(p.published_at), 'MMM dd')}` : ''}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
        ...
        {posts.length === 0 && <p className="text-center text-gray-400 py-8">No posts yet. Write your first one!</p>}
      </div>
    </div>
  );
};
...
const ProgramsTab = () => (
  <div>
    <h2 className="text-2xl font-extrabold mb-6">Programs</h2>
    <div className="bg-white rounded-xl p-6 border border-gray-100">
      <p className="text-gray-400 text-sm">Program management is coming soon. Currently, 8 micro-credential programs are configured.</p>
      <div className="mt-4 space-y-2">
        {['The Architect', 'Da Plotter', 'The Alchemist', 'The Gatekeeper', 'The Voice', 'The Narrator', 'The Lens', 'The Selector'].map((name, i) => (
          <div key={name} className="flex items-center justify-between px-4 py-3 bg-gray-50 rounded-lg">
            <div className="flex items-center gap-3">
...
const MediaTab = () => (
  <div>
    <h2 className="text-2xl font-extrabold mb-6">Media Gallery</h2>
