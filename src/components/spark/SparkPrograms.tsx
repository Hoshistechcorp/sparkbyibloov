...
const programs = [
  { id: '1', title: 'Advanced Event Planning & Creative Organizing', tag: 'FLAGSHIP', desc: 'Go beyond logistics — master strategic budgeting, risk management, and immersive concept development that transforms ordinary events into extraordinary, high-impact experiences.', color: '#c48500' },
  ...
  { id: '6', title: 'Storyteller — TikTok Content for Events', tag: 'TRENDING', desc: 'Create compelling short-form videos for events, venues, and hospitality brands. Storytelling, editing, trends, and monetization strategies.', color: '#ec9f00' },
...
export const SparkPrograms = () => {
  ...
  return (
    <section id="programs" className="py-24 px-6 md:px-12 max-w-7xl mx-auto">
      <div className="mb-12">
        <span className="text-[11px] tracking-[0.3em] uppercase text-[#c48500] font-bold mb-3 block">Micro-Learning Programs</span>
      ...
      <div className="space-y-3">
        {programs.map((p) => (
          <div key={p.id}
            className="group border border-gray-100 rounded-xl hover:border-[#ec9f00]/40 transition-all cursor-pointer bg-white hover:shadow-md"
            ...
            <div className="flex items-center gap-4 md:gap-6 p-5 md:p-6">
              ...
              <span className="text-[9px] tracking-[0.2em] uppercase font-extrabold px-2.5 py-1 rounded-full border"
                style={{ color: p.color === '#ec9f00' ? '#c48500' : p.color, borderColor: (p.color === '#ec9f00' ? '#c48500' : p.color) + '40', backgroundColor: (p.color === '#ec9f00' ? '#ec9f00' : p.color) + '10' }}>
                {p.tag}
              </span>
              <h3 className="text-sm md:text-base font-bold flex-1 group-hover:text-[#c48500] transition-colors text-gray-800">{p.title}</h3>
