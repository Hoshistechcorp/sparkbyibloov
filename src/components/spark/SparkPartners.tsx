...
export const SparkPartners = () => (
  <section id="partners" className="py-16 md:py-24 px-4 md:px-12 max-w-7xl mx-auto">
    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mb-12">
      <span className="text-[11px] tracking-[0.3em] uppercase text-[#c48500] font-bold mb-3 block">Partnerships</span>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
      {partnerTypes.map((p, i) => (
        <motion.div key={p.type} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}
          whileHover={{ y: -6 }}
          ...
          <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: 0.3 + i * 0.1, type: 'spring' }}
            className="w-3 h-3 rounded-full mb-4" style={{ backgroundColor: p.accent }} />
          ...
          <ul className="space-y-2 mb-8 flex-1">
            {p.benefits.map((b, bi) => (
              <motion.li key={b} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.3, delay: 0.4 + bi * 0.1 }}
                className="flex items-center gap-2 text-sm text-gray-500">
                <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: p.accent }} />{b}
              </motion.li>
            ))}
          </ul>
