  ...
  <section id="dna" className="py-16 md:py-24 px-4 md:px-12 bg-gray-50">
    <div className="max-w-7xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mb-12">
        <span className="text-[11px] tracking-[0.3em] uppercase text-[#c48500] font-bold mb-3 block">Platform DNA</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
        {pillars.map((p, i) => (
          <motion.div key={p.source} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}
            whileHover={{ y: -8, boxShadow: '0 25px 50px -15px rgba(0,0,0,0.08)' }}
            className="bg-white border border-gray-100 p-5 md:p-8 rounded-2xl hover:border-[#ec9f00]/40 transition-all group cursor-pointer relative overflow-hidden">
            <motion.div initial={{ width: 0 }} whileInView={{ width: '60px' }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 + i * 0.1 }}
              className="h-1 rounded-full mb-6" style={{ backgroundColor: p.accent }} />
            <div className="text-[10px] tracking-[0.2em] uppercase text-gray-300 font-bold mb-1">Inspired by {p.source}</div>
            <h3 className="text-xl font-bold mb-3 group-hover:text-[#c48500] transition-colors text-gray-900">{p.quality}</h3>
