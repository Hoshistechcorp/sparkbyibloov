...
const audiences = [
  ...
  {
    id: 'partners', label: 'Training Partners', tagline: 'Scale your impact. We handle the platform.',
    people: [
  ...
  {
    id: 'experts', label: 'Leading Experts', tagline: 'Your knowledge deserves a bigger stage.',
    people: [
  ...
  return (
    <section id="audiences" className="py-16 md:py-24 px-4 md:px-12 max-w-7xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mb-12">
        <span className="text-[11px] tracking-[0.3em] uppercase text-[#c48500] font-bold mb-3 block">Built for everyone</span>
        <h2 className="text-3xl md:text-5xl font-extrabold tracking-tight text-gray-900">Who is Spark for?</h2>
      </motion.div>

      <div className="flex flex-wrap gap-2 mb-10">
        {audiences.map((a) => (
          <motion.button key={a.id} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setActive(a.id)}
            className={`text-[12px] tracking-[0.1em] uppercase px-5 py-2.5 rounded-full font-bold transition-all ${
              active === a.id ? 'bg-[#ec9f00] text-white shadow-md shadow-[#ec9f00]/20' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'
            }`}>
            {a.label}
          </motion.button>
        ))}
        ...
        <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-5">
            {current.people.map((person, i) => (
              <motion.div key={person.name} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.1 }}
                whileHover={{ y: -8 }}
                className="group bg-white border border-gray-100 rounded-2xl overflow-hidden hover:shadow-xl hover:shadow-[#ec9f00]/10 transition-all cursor-pointer">
                <div className="relative h-36 md:h-52 overflow-hidden">
                  <motion.img src={person.image} alt={person.name} className="w-full h-full object-cover" whileHover={{ scale: 1.08 }} transition={{ duration: 0.6 }} />
